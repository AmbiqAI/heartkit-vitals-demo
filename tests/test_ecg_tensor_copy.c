...
/* CHECK_EQ truncates to long long, so scores are compared scaled. A power of
 * two keeps every scaled value exact. */
#define SEG_QOS_SCALE (1048576.0f)
...
test_denoise_output_f32(void) {
    ...
    for (int i = 0; i < ECG_DEN_WINDOW_LEN; i++) { CHECK_EQ(host[i], (float)i); }
    ...
    for (int i = 0; i < ECG_DEN_PAD_LEN; i++) { CHECK_EQ(host[i], HOST_SENTINEL); }
    for (int i = ECG_DEN_PAD_LEN; i < ECG_DEN_WINDOW_LEN - ECG_DEN_PAD_LEN; i++) { CHECK_EQ(host[i], (float)i); }
    for (int i = ECG_DEN_WINDOW_LEN - ECG_DEN_PAD_LEN; i < ECG_DEN_WINDOW_LEN; i++) {
        CHECK_EQ(host[i], HOST_SENTINEL);
    }
...
test_denoise_output_i8(void) {
    ...
    for (int i = 0; i < ECG_DEN_WINDOW_LEN; i++) {
        CHECK_EQ(host[i] * 128.0f, (float)((int)tensor[i] - Q_ZERO));
    }
...
test_segmentation_output_mask(void) {
    ...
    CHECK_EQ(avgQos * SEG_QOS_SCALE, SEG_WIN_QOS * SEG_QOS_SCALE);
...
test_input_tail_is_defined(void) {
    ...
    for (int i = 0; i < ECG_DEN_WINDOW_LEN; i++) { CHECK_EQ(tensorF[i], host[i]); }
    for (int i = ECG_DEN_WINDOW_LEN; i < MODEL_LEN; i++) { CHECK_EQ(tensorF[i], host[ECG_DEN_WINDOW_LEN - 1]); }
...
test_narrower_model_is_clamped(void) {
    ...
    for (int i = 0; i < narrow; i++) { CHECK_EQ(host[i], (float)i); }
    for (int i = narrow; i < ECG_DEN_WINDOW_LEN; i++) { CHECK_EQ(host[i], HOST_SENTINEL); }
    ...
    for (int i = 0; i < narrow; i++) { CHECK_EQ(tensor[i], host[i]); }
