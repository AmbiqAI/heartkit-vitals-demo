...
For each log written by `swo_capture.py`, prints:
  ...
  - distributions (n, min, median, max) of ECG quality `qos`, model
    `cossim`, heart rate, CPU `util`, `cpu_proj` (deployment projection)
    and `avg_ips`, plus TileIO `qdepth`;
...
# Measured and projected are reported separately and must be read that way. See #8.
...
DIST_KEYS = ["qos", "cossim", "hr", "util", "proj", "ips", "qdepth"]
