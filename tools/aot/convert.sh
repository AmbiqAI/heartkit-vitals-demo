...
# generation timestamp in the file banners, which --check ignores. The banners
# differ in shape: sources carry `@date`, the README a markdown `Generated by
# **heliaAOT** (vX) on <date>` line, so the ignore pattern has to span both.
set -euo pipefail
