...
# BSD-3-Clause requires the licence text and attribution to accompany the
# binary, so they ship in the package root too. See #70.
for _f in LICENSE NOTICE; do
  [ -f "${REPO_DIR}/${_f}" ] \
    || die "${_f} not found at the repo root (${REPO_DIR}); the package must ship it"
  cp -p "${REPO_DIR}/${_f}" "${PKG_ROOT}/${_f}" \
    || die "could not copy ${_f} into ${PKG_ROOT}"
  chmod 644 "${PKG_ROOT}/${_f}"
done
...
for _d in "${BOARD_DIRS[@]}"; do
  ...
  # The licence, attribution and notices cover the whole module closure, which
  # is the same for every board in the drop, so each per-board archive carries
  # the same files.
  cp -p "${PKG_ROOT}/THIRD-PARTY-NOTICES.md" "${_stage}/${TAG}/"
  cp -p "${PKG_ROOT}/LICENSE" "${PKG_ROOT}/NOTICE" "${_stage}/${TAG}/"
